...
G4VPhysicalVolume* myDetectorConstruction::Construct()
{
  ...
  //
	...
	G4double pRmax = 4*km;
	G4double pDz   = 2*km; // half-length

  // TUBS
	G4double world_pRmin = 0.0;
	G4double world_pRmax = 1.1*pRmax;
	G4double world_pDz =   1.1*pDz; // half-length
  ...
  //
	...
	G4double ground_pRmax = 3*km;
	G4double ground_pDz =   50*m; // half-length
	G4double ground_pSPhi = 0.0; 
	G4double ground_pDPhi = 2.0*pi;

	G4double ground_z = -(pDz + ground_pDz); 
	...
	G4LogicalVolume** logicWorld)
{
	...
	G4double slab_pRmax = 3*km; // radius
